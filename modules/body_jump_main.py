...
class JumpingJackDetector(BaseDetector):
    ...
    def __init__(self, config=None, language='ZH_TW'):
        ...
        Args:
            config: (Optional) 包含角度設定的字典,若為 None 則使用預設值
            language: 語言設定 (預設 'ZH_TW')
        """
        super().__init__()
        
        # ========== 載入多語言支援 ==========
        self.lang = LanguageLoader('body_jump_main', language)
        ...
        self.cfg = {
            'arm_down_angle': 45,
            'arm_up_angle': 155,
            'elbow_straight_angle': 90,
            'knee_straight_angle': 145,
            'knee_strict_angle': 165,
            'leg_in_angle': 95,    # 新增: 雙腳合攏時的髖部角度 (直立約90度)
            'leg_out_angle': 110,  # 新增: 雙腳張開時的髖部角度
            'auto_pause_seconds': 5.0  # 預設的暫停秒數
        }
        
        # 2. 如果有外部 config 傳入,則覆蓋預設值
        if config:
            self.cfg.update(config)
            print(self.lang.log('info', 'config_loaded'))
        
        # 3. 將設定值綁定到實例變數
        self.arm_down_angle = self.cfg['arm_down_angle']
        self.arm_up_angle = self.cfg['arm_up_angle']
        self.elbow_straight_angle = self.cfg['elbow_straight_angle']
        self.knee_straight_angle = self.cfg['knee_straight_angle']
        self.knee_strict_angle = self.cfg['knee_strict_angle']
        self.leg_in_angle = self.cfg['leg_in_angle']    
        self.leg_out_angle = self.cfg['leg_out_angle']  
        self.auto_pause_seconds = self.cfg['auto_pause_seconds']
        
        # ========== 狀態變數 ==========
        self.A = False
        self.B = False
        self.L_arm_wrong = False
        self.R_arm_wrong = False
        self.L_knee_wrong = False
        self.R_knee_wrong = False
    ...
    def get_detailed_errors(self):
        """回傳原始的變數名稱，作為 JSON 分析用"""
        # 直接回傳 total_errors 中大於 0 的項目，不轉換為中文
        return {k: v for k, v in self.total_errors.items() if v > 0}
    ...
    def process_frame(self, img, landmarks):
        ...
        # ========== 2. 新增(容錯版)：檢查半邊身體是否入鏡 ==========
        def check_visible(lm):
            """檢查單一關鍵點是否在畫面內且可見度大於 0.6"""
            return (lm.visibility >= 0.6 and 
                    0.0 <= lm.x <= 1.0 and 
                    0.0 <= lm.y <= 1.0)

        # 頭部 (鼻子:0)
        head_visible = check_visible(landmarks[0])
        # 手部 (左手腕:15 或 右手腕:16 擇一即可)
        hand_visible = check_visible(landmarks[15]) or check_visible(landmarks[16])
        # 腳部 (左腳踝:27 或 右腳踝:28 擇一即可)
        foot_visible = check_visible(landmarks[27]) or check_visible(landmarks[28])
        
        # 只要滿足頭、至少一手、至少一腳有在畫面內，就算有入鏡
        body_fully_visible = head_visible and hand_visible and foot_visible
                
        detection_info['body_visible'] = body_fully_visible
        
        if not body_fully_visible:
            # 如果連半邊身體都沒入鏡，直接暫停計時，不進入動作判斷邏輯
            self._pause_timer()
        else:
            # ========== 計算各關節角度 (僅在有效入鏡時執行) ==========
            # 右手肘 (11-13-15)
            ...
            #========== 新增: 髖部角度 (腿部張開幅度) ==========
            # 右側髖部 (23-24-26)
            a7, b7, c7 = get_xy(23), get_xy(24), get_xy(26)
            angle_24 = utils_math.calculate_angle(a7, b7, c7)
            detection_info['angles']['right_hip'] = angle_24
            cv2.putText(img, f'{int(angle_24)}', (int(b7[0]), int(b7[1]-20)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)

            # 左側髖部 (24-23-25)
            a8, b8, c8 = get_xy(24), get_xy(23), get_xy(25)
            angle_23 = utils_math.calculate_angle(a8, b8, c8)
            detection_info['angles']['left_hip'] = angle_23
            cv2.putText(img, f'{int(angle_23)}', (int(b8[0]), int(b8[1]-20)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)

            # ========== 動作邏輯判斷 ==========
            # 將手腳狀態提取為布林值，增加程式可讀性
            arms_down = angle_12 <= self.arm_down_angle and angle_11 <= self.arm_down_angle
            arms_up = angle_12 >= self.arm_up_angle and angle_11 >= self.arm_up_angle
            legs_closed = angle_24 <= self.leg_in_angle and angle_23 <= self.leg_in_angle
            legs_spread = angle_24 >= self.leg_out_angle and angle_23 >= self.leg_out_angle

            # 判斷預備動作 (必須: 手放下 且 腳合攏)
            if arms_down and legs_closed:
                if not self.A:
                    self.A = True
                    self.B = False
                    self._resume_timer()  # 動作觸發，開始計時或重置閒置時間
            
            # 判斷最高點動作 (必須: 手舉起 且 腳張開)
            ...
            # 錯誤判斷
            if angle_13 <= self.elbow_straight_angle: self.L_arm_wrong = True
            if angle_14 <= self.elbow_straight_angle: self.R_arm_wrong = True
            
            if angle_25 <= self.knee_straight_angle: self.L_knee_wrong = True
            if angle_26 <= self.knee_straight_angle: self.R_knee_wrong = True
            
            # 完整動作判斷
            if (self.A and self.B) and \
               arms_down and legs_closed and \
               angle_26 >= self.knee_strict_angle and angle_25 >= self.knee_strict_angle:
                ...
                # [修改] 檢查四個部位是否任一有錯
                if self.L_arm_wrong or self.R_arm_wrong or self.L_knee_wrong or self.R_knee_wrong:
                    self.wrong += 1
                    detection_info['form_correct'] = False
                    
                    if self.L_arm_wrong:
                        self.total_errors["fail_L_arm"] += 1
                        print(self.lang.log('error', 'fail_L_arm'))
                        cv2.circle(img, (int(b[0]), int(b[1])), 10, (0, 0, 255), -1)  # 左手肘 (13)
                        
                    if self.R_arm_wrong:
                        self.total_errors["fail_R_arm"] += 1
                        print(self.lang.log('error', 'fail_R_arm'))
                        cv2.circle(img, (int(b2[0]), int(b2[1])), 10, (0, 0, 255), -1) # 右手肘 (14)
                    
                    if self.L_knee_wrong:
                        self.total_errors["fail_L_knee"] += 1
                        print(self.lang.log('error', 'fail_L_knee'))
                        cv2.circle(img, (int(b5[0]), int(b5[1])), 10, (0, 0, 255), -1) # 左膝蓋 (25)
                        
                    if self.R_knee_wrong:
                        self.total_errors["fail_R_knee"] += 1
                        print(self.lang.log('error', 'fail_R_knee'))
                        cv2.circle(img, (int(b6[0]), int(b6[1])), 10, (0, 0, 255), -1) # 右膝蓋 (26)
        ...
        if not body_fully_visible:
            try:
                # 嘗試抓取多語言字典內的警告，若無則顯示英文預設值
                warning_msg = self.lang.get_message('warning_full_body')
                if "Missing" in warning_msg: warning_msg = "Please Show Full Body"
            except:
                warning_msg = "Please Show Full Body"
                
            # 在畫面中央提示使用者調整距離或角度
            img = self.draw_text(img, f"[{warning_msg}]", (w // 2 - 180, h // 2), (0, 0, 255), 40)
...
if __name__ == "__main__":
    ...
    lang = LanguageLoader('body_jump_main', LANGUAGE)
    ...
    try:
        ...
        # 初始化邏輯
        jump_logic = JumpingJackDetector(language=LANGUAGE)
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                frame, info = jump_logic.process_frame(frame, results.pose_landmarks[0])
            ...
            cv2.imshow('Jumping Jack Counter (Debug)', frame)
        ...
        if 'jump_logic' in locals():
            print(f"\n{lang.get_message('final_stats')}")
            print(f"{lang.get_message('total_count')}: {jump_logic.count}")
            print(f"{lang.get_message('correct_count')}: {jump_logic.correct}")
            print(f"{lang.get_message('wrong_count')}: {jump_logic.wrong}")
            try:
                time_lbl = lang.get_message('total_time')
                if "Missing" in time_lbl: time_lbl = "Total Time"
            except:
                time_lbl = "Total Time"
            print(f"{time_lbl}: {jump_logic.get_total_time():.1f} s")